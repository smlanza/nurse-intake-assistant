...
- Cosmos case list intake-status filter parity is complete.
- `CosmosCaseRepository.list_cases(...)` queries existing `/createdDate`
  partitions newest-first with parameterized review, urgency, intake-status, and date filters.
...
- Fake tests cover intake-status/query combinations offline; no live Cosmos validation is claimed.
...
- Cosmos repository boundary, manual Cosmos point-read path, and offline-tested
  ...
  urgency, intake-status, and inclusive from/to date filters
...
- `APP_MODE=cosmos` uses `CosmosCaseRepository` and requires Cosmos settings.
  ...
  review-status, urgency, intake-status, and date filters. Remaining mock/Cosmos
