...
- Cosmos case list notification-email-status filter parity is complete.
- `CosmosCaseRepository.list_cases(...)` queries existing `/createdDate`
  partitions newest-first with parameterized review-status, urgency, intake-status, intake-complete, source-system, case-type, notification-email-status, and date filters.
...
- Fake tests cover the flat persisted email-status field offline; no live Cosmos validation is claimed.
...
- Cosmos repository boundary, manual Cosmos point-read path, and offline-tested
  ...
  urgency, intake-status, intake-complete, source-system, case-type, notification-email-status, and inclusive date filters
...
- `APP_MODE=cosmos` uses `CosmosCaseRepository` and requires Cosmos settings.
  ...
  review-status, urgency, intake-status, intake-complete, source-system, case-type, notification-email-status, and date filters. Remaining mock/Cosmos
