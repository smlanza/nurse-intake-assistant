...
- `HostedFoundryAgentSshTransport` and its wrapper preserve the live-proven one-tunnel lifecycle, authoritative loopback readiness proof, two fixed `APP_PATH` probes, private output and host-key handling, and guaranteed interrupt/terminate/kill reaping. The existing `--live-tunnel` mode remains non-invoking. Metadata mode adds one distinct default-no approval and exactly one packaged read-only metadata verifier command; its exact success contract prohibits Agent invocation and Azure mutation. The canonical five-setting declaration now owns infrastructure, CLI, verification, packaged-runtime, and SSH assignment order. The metadata-only SSH extension is implemented offline and awaiting separate supervised live acceptance.
- Metadata-only SSH consumes the exact Web App configuration proof before constructing an immutable private runtime configuration. Only the five validated canonical values enter the fixed metadata command; no arbitrary local environment value or App Service-owned identity marker is read or propagated. Invalid configuration fails before SSH service or tunnel construction. Values, the rendered command, and raw remote output remain absent from prompts and serialized results. Exact authoritative verifier failures retain only the existing allowlisted category; every malformed or additional-output shape remains generic and fail-closed. This correction made no Azure, SSH, managed-identity, metadata, Agent, deployment, mutation, or network call.
Architecture impact: updated the existing hosted Foundry SSH transport section because the durable control-plane-to-runtime verifier configuration flow changed.
