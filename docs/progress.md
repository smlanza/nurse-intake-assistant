...
- Cosmos case list source-system filter parity is complete.
- `CosmosCaseRepository.list_cases(...)` queries existing `/createdDate`
  partitions newest-first with parameterized review, urgency, intake, source, and date filters.
...
- Fake tests cover source-system/query combinations offline; no live Cosmos validation is claimed.
...
- Cosmos repository boundary, manual Cosmos point-read path, and offline-tested
  ...
  urgency, intake-status, source-system, and inclusive from/to date filters
...
- `APP_MODE=cosmos` uses `CosmosCaseRepository` and requires Cosmos settings.
  ...
  review, urgency, intake, source-system, and date filters. Remaining mock/Cosmos
