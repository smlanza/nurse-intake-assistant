...
- `src/app/services/web_app_infra_deployment.py`: sanitized, purpose-bound initial-create and reconciliation contracts; its generic orchestrator rejects the `web_app_authentication` purpose. `scripts/deploy_web_app_infra.py` provides offline check and explicit what-if/live operation with nondefault `--reconcile-existing-web-app` selection. `scripts/accept_web_app_authentication.py` is the dedicated guarded existing-parent Authentication workflow; its normal supervised live path, bounded configuration/perimeter proofs, and interactive protected-page acceptance are proven for one current generation. Application authorization remains separate and deferred.
...
- The standalone Application Insights smoke now establishes a private bounded current-attempt scope before its single fixed-fictional emission. The strict verifier remains authoritative and runs first; only one uniquely attributable expected-name row rejected as `telemetry_record_invalid` is passed in memory to the existing sanitized classifier during the same invocation, and the public result exposes only whether diagnosis ran and its allowlisted field, mismatch-reason, and wire-type enums.
  No raw row, value, identifier, private scope material, or unrestricted Azure output is persisted or returned. Absent, ambiguous, malformed, query, identity, and readiness failures remain fail-closed without diagnosis. RED was 6 failed and 20 passed when the result contract lacked diagnostic fields; focused GREEN is 48 passed and the broader telemetry/application-processing regression is 66 passed.
  One fresh supervised live attempt completed the production-composed in-memory path with notifications suppressed and exactly one emission, then strict verification returned `telemetry_record_invalid`. Same-attempt diagnosis safely returned no affected field, `record_shape_invalid`, and wire type `string`; no retry, Azure mutation, or raw/private telemetry exposure or persistence occurred. This indicates that the proof parser receives the projected dimensions envelope as a string where it requires an object; the smallest likely next correction is strict one-time dynamic-column JSON decoding at that existing parser boundary, without changing the canonical event model or verifier semantics. Standalone live telemetry ingestion and live App Service-hosted telemetry remain unproven; the latter is outside this slice.
