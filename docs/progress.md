...
impossible. Operator-supervised direct App Service SSH transport, its two fixed `APP_PATH` probes, and the packaged non-invoking check are live-proven. The first metadata-only attempt returned `missing_configuration` before managed-identity construction. Exact five-setting propagation corrected that configuration boundary; a subsequent attempt returned `not_running_in_hosted_environment`, proving the SSH process lacks the application-worker identity environment. Managed identity was never attempted, so neither result establishes an RBAC, token, Foundry authorization, or model-access failure. SSH managed-identity execution is retired, hosted metadata access and invocation remain unproven, and no replacement mechanism was selected. Separately, application-integrated Microsoft Foundry Agent execution and application-integrated structured extraction remain live-proven.
