...
- The shared cleanup service now accepts a structurally valid successful Speech purge result without requiring stdout JSON, preserves boolean mutation accounting, and still requires final resource-group, Foundry-tombstone, and Speech-tombstone absence. Offline correction verification is complete; fresh daily READY remains required before SSH transport acceptance resumes.
