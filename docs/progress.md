...
sequence. Direct operator-supervised App Service SSH is the selected future
execution transport for the packaged synchronous proof operation.
...
The packaged synchronous hosted proof operation remains the fixed non-invoking remote boundary. A prior acceptance stopped before Azure or SSH, and the repository retains its offline-tested one-tunnel lifecycle. The narrow SSH transport/prerequisite acceptance later advanced to one fresh supervised SSH transport acceptance that started exactly one tunnel but ended with sanitized `tunnel_outcome_ambiguous` before readiness; live SSH transport remains unproven. The supported readiness-observation defect is corrected offline. The next operator slice requires fresh current-artifact READY evidence followed by one separately approved supervised SSH transport acceptance retry, stopping after transport acceptance; managed-identity metadata access and Agent invocation require a later slice.

## Current Slice Status

- Fresh matching READY evidence preceded one supervised direct App Service tunnel attempt. Exactly one process started, readiness remained unproven, and the historical result remains the failure `tunnel_outcome_ambiguous`; no remote command, metadata read, or Agent invocation ran. Interrupt, private host-key cleanup, and process reaping succeeded, and sanitized inspection found no matching process. No retry or alternate transport occurred.
...
- `HostedFoundryAgentSshTransport` and its wrapper own one fixed tunnel process, bounded readiness observation, three approvals, exactly two `APP_PATH` probes and one packaged check, private output and host-key handling, and guaranteed interrupt/terminate/kill reaping. The offline correction now makes the owned loopback listener proof authoritative without requiring useful or complete CLI output, while preserving the absolute deadline and fail-closed process classification. Check mode remains deterministic and constructs no subprocess, socket, SSH client, credential, Azure runner, or host-key material. No live tunnel or remote command ran during the correction; fresh READY evidence and a separately approved retry remain required, and hosted managed-identity metadata access and invocation remain unproven.
