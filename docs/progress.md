...
The daily-generation Key Vault runtime RBAC architecture is live-proven for the current August 17 generation. Initial application infrastructure creates the Web App and zero-secret RBAC-mode vault without the runtime role assignment. The coordinator proved the exact current vault and Web App system identity, established the exact Key Vault Secrets User assignment required by the daily contract, and independently verified it before READY. Exact runtime RBAC orchestration and READY gating are implemented offline and now accepted live for one fresh daily generation. The principal remains private and generation-bound, so identity churn cannot reuse yesterday's proof. Operator Key Vault Reader remains unproven; its live daily acceptance remains unproven. Zero-secret metadata proof, live secret retrieval, credential migration, App Service secret references, and production secret operations also remain separate and unproven. Consumer RBAC remains among the standalone optional workflows outside READY. WebJob evidence recovery remains exceptional, and the trigger-and-correlation path is retired and must not be reused without a new explicit architecture decision.
...
**Current Authentication state:** Initial opt-in remains disabled by default. A fresh daily environment reached READY, and the normal supervised `accept_web_app_authentication.py --live` path re-established App Service Authentication v2 for the current August 17 generation. A safe preview and explicit operator approval preceded the Authentication-only mutation; the exact existing Microsoft Entra application and tenant then passed configuration verification. The current-generation runtime perimeter returned `authentication_perimeter_verified`: `/health`, `/version`, and `/demo/status` are anonymous, while `/demo`, `/cases`, `/docs`, and `/openapi.json` are protected. No `--repair-live` path was used and no FastAPI authentication layer was added.
The fresh disabled `authsettingsV2` read returned `excludedPaths: null`, exposing a list-only disabled-state parser defect. RED reproduced the rejection; the smallest focused correction and GREEN verification now accept only null or `list[str]` in the disabled-state parser while preserving exact `platformEnabled is False` and all scalar checks. The stricter configured-state diagnostic/parser remains unchanged.
No Entra app registration was created; the existing registration was retained. ID-token issuance was enabled, Access-token issuance remained disabled, and the standard App Service `/.auth/login/aad/callback` reply path was added for the current Web App. Sequential sanitized diagnosis first identified `AADSTS700054` for disabled ID-token issuance and then `AADSTS500113` for the missing reply address. After those narrow corrections, one fresh operator-supervised App Service Entra sign-in succeeded and protected `GET /demo` loaded; `authenticated_application_access_verified` is proven. This proves interactive authentication, not application authorization: roles, groups, nurse authorization, API authorization, and broader authorization policy remain deferred. No client secret, certificate, API permission, tenant-wide configuration, or further `authsettingsV2` change was made. No hostname, tenant or application identifier, request or correlation identifier, token, cookie, credential, or session value is recorded.
Mock mode and no-op intake telemetry remain the safe defaults, hosted notifications remain suppressed, and all AI output continues to require human nurse review.
...
Authentication configuration, current-generation perimeter verification, and one interactive protected-page sign-in are complete for the August 17 generation. Do not repeat or broaden Authentication changes for this generation absent separately diagnosed drift. A rebuilt disposable environment must regain fresh READY, configuration, perimeter, and generation-bound evidence through the normal supervised path. Any application authorization work is a separately frozen future slice; roles, groups, nurse authorization, API authorization, and broader policy remain deferred, and FastAPI must not gain a parallel authentication layer implicitly.
The application-owned Key Vault boundary includes one approved live deployment of the exact repository-owned vault. Independent control-plane verification proved the Azure-returned identity, successful provisioning, Azure RBAC mode, and no legacy access policies. The current Web App system identity now also has an independently verified exact Key Vault Secrets User assignment, closing live daily-generation runtime assignment acceptance. The earlier bounded metadata-only zero-secret check still failed closed as `secret_metadata_read_failed`; the fixed-role operator Key Vault Reader workflow remains unproven after sanitized `authorization_scope_mismatch`, and zero-secret proof remains unknown. No secret was created, retrieved, deleted, or migrated; live retrieval, credential migration, App Service secret references, and production secret operations remain deferred. Azure Speech closure is complete. Bounded OneDeploy and hosted-readiness convergence are complete offline. A READY receipt remains valid only while environment and configuration match; deletion or rebuild invalidates it.
...
The final fresh disposable generation reached READY, including live-proven Foundry infrastructure, prompt-agent configuration, immutable routing, Web App configuration, current application deployment and artifact equality, and hosted readiness. Generation handoff then succeeded, the fixed triggered WebJob was discovered, and the direct project-scoped Consumer assignment was reused and verified without mutation.
...
- `infra/modules/web-app.bicep`: reusable initial-create module and direct existing-Web-App reconciliation boundary with offline-tested Linux hosting, system-assigned identity, and remote build; initial Authentication opt-in invokes the single authoritative `infra/modules/web-app-authentication.bicep`, whose existing-parent boundary can deploy only `authsettingsV2`. Existing-Web-App telemetry instead uses `infra/modules/web-app-telemetry.bicep`, whose only deployable resource is the existing Web App's `appsettings` child and whose existing Application Insights reference resolves the private connection string internally.
- `src/app/services/web_app_infra_deployment.py`: sanitized, purpose-bound initial-create, reconciliation, and hosted-telemetry configuration contracts; its generic orchestrator rejects the `web_app_authentication` purpose. `scripts/deploy_web_app_infra.py` provides offline check and explicit what-if/live operation with nondefault, mutually exclusive reconciliation and hosted-telemetry selections. Broad reconciliation cannot enable hosted telemetry. `scripts/accept_web_app_authentication.py` is the dedicated guarded existing-parent Authentication workflow; its normal supervised live path, bounded configuration/perimeter proofs, and interactive protected-page acceptance are proven for one current generation. Application authorization remains separate and deferred.
- `src/app/services/web_app_hosting_contract.py`: exact eight-setting default hosted contract, including `TELEMETRY_PROVIDER=none`, plus separate explicit hosted-verifier and Azure Monitor telemetry opt-ins shared by infrastructure deployment and configuration verification. The telemetry opt-in resolves the existing Application Insights configuration inside Bicep and never accepts or returns it through the CLI.
...
- ACS phone intake/call automation; operator Key Vault Reader, zero-secret
  metadata proof, live secret retrieval, migration of current credentials, and
...
- Optional Key Vault exact-name secret-access boundary, RBAC-mode infrastructure, and a live-proven daily-generation exact-vault-scope Key Vault Secrets User assignment for the current Web App identity
- Live-proven App Service Authentication configuration, runtime perimeter, and one operator-supervised interactive protected-page acceptance; no parallel FastAPI authentication or application authorization was added
- No Azure calls in tests, PHI, production clinical behavior, live secret retrieval or credential migration, phone intake automation, retry/durable processing, or production frontend work were added.
...
- The Web App infrastructure CLI uses `main.bicep` for initial creation, the direct Web App module for existing-site reconciliation, and the dedicated existing-parent app-settings entry point for hosted telemetry; none creates the resource group.
...
- The current daily-generation Web App system identity received and independently passed exact-vault-scope verification for the required Key Vault Secrets User assignment.
...
- Application authorization, including roles, groups, nurse authorization, API authorization, and broader authorization policy
- Agent-specific RBAC scope
- Live hosted managed-identity verification and agent invocation
- Operator Key Vault Reader, zero-secret metadata proof, live secret retrieval, current-credential migration, App Service secret references, and production secret rotation/operations
...
App Service Authentication acceptance and the daily Key Vault runtime assignment are complete for the current generation; no corrective Authentication or Key Vault deployment slice remains. Select the next Nurse Intake Assistant task through a separately frozen slice. Application authorization and the remaining Key Vault reader, metadata, retrieval, migration, reference, and production-operation boundaries remain deferred. The packaged synchronous hosted proof operation remains execution-mechanism-neutral, direct App Service SSH transport is live-proven and non-invoking only, SSH hosted managed-identity execution is unsupported, and no replacement hosted execution topology is selected.

## Current Slice Status

- The August 17 daily environment reached fresh READY. The normal supervised `accept_web_app_authentication.py --live` path used a safe preview and explicit operator approval before one Authentication-only mutation, then verified the exact existing Entra application/tenant configuration. Current perimeter proof returned `authentication_perimeter_verified` with `/health`, `/version`, and `/demo/status` anonymous and `/demo`, `/cases`, `/docs`, and `/openapi.json` protected. `--repair-live` was not used, and no FastAPI authentication layer or further `authsettingsV2` change was required.
- A fresh disabled `authsettingsV2` state exposed `excludedPaths: null`; the prior disabled-state parser rejected it. RED reproduced the defect and focused GREEN passed after the parser alone accepted null or `list[str]`, retaining exact `platformEnabled is False`, scalar checks, and the unchanged stricter configured-state parser/diagnostic. Architecture impact: none.
- The existing Entra registration was retained. ID-token issuance was enabled, Access-token issuance stayed disabled, and a callback/reply URI using the standard `/.auth/login/aad/callback` path was added. Sanitized sequential diagnosis recorded `AADSTS700054` and then `AADSTS500113`; one later fresh operator-supervised sign-in loaded protected `GET /demo`, proving `authenticated_application_access_verified`. This does not prove application authorization. No new registration, secret, certificate, API permission, tenant-wide configuration, identifier, token, cookie, credential, or session value was added to repository evidence.
- The current Web App system identity received and independently verified the exact Key Vault Secrets User assignment required by daily runtime RBAC. Live daily-generation runtime assignment acceptance is now proven; operator Key Vault Reader, zero-secret metadata proof, live retrieval, credential migration, App Service secret references, and production secret operations remain separate and unproven.
- The standalone Application Insights smoke establishes a private bounded current-attempt scope before its single fixed-fictional emission. The strict verifier remains authoritative and runs first; only one uniquely attributable expected-name row rejected as `telemetry_record_invalid` reaches the existing same-attempt sanitized classifier, whose public result is limited to allowlisted field, mismatch-reason, and wire-type enums.
  The existing proof parser now leaves object-shaped projected dimensions unchanged and decodes a string envelope exactly once, requiring the decoded top-level value to be an object before applying every existing strict record check. Malformed, non-object, and double-encoded JSON fail closed, and no raw row, envelope, value, identifier, private scope material, or unrestricted Azure output is persisted or returned. RED was one failed focused JSON-object-envelope proof; GREEN is 32 parser/proof tests, 85 broader Application Insights tests, and 35 application-processing tests.
  One fresh supervised acceptance revalidated current READY and the exact receipt-bound identity, completed the production-composed in-memory path with notifications suppressed, emitted exactly once, and strictly verified exactly one eligible record with the full allowlisted contract; diagnosis did not run. No retry, Azure mutation, or raw/private telemetry exposure or persistence occurred. Standalone fixed-fictional Application Insights ingestion is live-proven. App Service-hosted Azure Monitor configuration now has a distinct offline-compiled existing-Web-App app-settings deployment boundary; the broader reconciliation surface remains unsuitable and cannot enable telemetry. No telemetry-specific live preview, deployment, emission, or query has occurred. Live configuration acceptance and hosted telemetry ingestion remain unproven; the next step is one supervised telemetry-specific what-if against the current READY generation.
...
- Historical Web App reconciliation proved too broad for hosted telemetry; the wrapper is removed,
  `--reconcile-existing-web-app` remains standalone, and telemetry opt-in now fails locally unless it selects the dedicated app-settings boundary. No telemetry-specific live preview or deployment has occurred.
