...
| Managed-identity and RBAC readiness | Optional IaC defines a Linux Web App system identity and separate project-scoped Consumer assignment. Deployment, assignment proof, and the App Service-hosted verification command remain distinct. That command verifies a Foundry prompt agent—not the Foundry Hosted Agents product runtime—using only `ManagedIdentityCredential`, one agent read, and one exact-version read. It requires all three App Service identity markers and closes its client and credential | `infra/modules/web-app.bicep`, `src/app/services/foundry_agent_consumer_rbac_verification.py`, `src/app/services/hosted_foundry_agent_verification.py`, `src/app/operations/verify_hosted_foundry_agent.py`, `tests/test_hosted_foundry_agent_verification.py`, `tests/test_verify_hosted_foundry_agent_operation.py` | All three boundaries are offline-tested only. No live RBAC deployment/read, managed-identity authentication, Foundry metadata read, inference, or invocation occurred; no hosted authentication, authorization, production, or clinical claim is made. Least privilege, human review, mock defaults, and suppressed hosted notifications remain unchanged |
...
- Live RBAC deployment, live read-only assignment verification, hosted
  managed-identity metadata verification, and later separate invocation
