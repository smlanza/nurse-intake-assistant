...
| Repeatable application deployment readiness | A read-only Azure CLI verifier checks an existing Web App against the Bicep-owned hosting, security, identity-presence, and safe-provider contract; an allowlist-driven service creates deterministic source ZIPs; a separate CLI submits only an explicit code-deployment request; and the hosted verifier checks `/health`, `/version`, and `/demo/status` | `infra/modules/web-app.bicep`, `src/app/services/web_app_configuration_verification.py`, `scripts/verify_web_app_configuration.py`, `src/app/services/web_app_package.py`, `scripts/deploy_web_app_code.py`, `src/app/services/web_app_readiness_verification.py`, `scripts/verify_web_app_readiness.py`, `tests/test_web_app_configuration_verification.py`, `tests/test_web_app_readiness_verification.py` | Configuration verification, packaging, deployment-command behavior, and hosted readiness are offline-tested only with fakes. Check modes make no Azure or HTTP call; live modes are separate and explicit. No live Web App configuration read, code deployment, hosted request, RBAC, managed-identity authentication, Foundry verification, or invocation occurred; no production-readiness claim is made |
...
- Web App infrastructure, its remote-build setting, deterministic packaging,
  read-only configuration verifier, explicit code-deployment request, and
  read-only hosted-readiness verifier are represented and offline-tested; live
  configuration verification, deployment, and readiness execution are deferred
