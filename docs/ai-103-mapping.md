...
| Azure AI Foundry / agent orchestration readiness | `FoundryAiService` and `NurseIntakeAgent` provide tested runtime boundaries and application-owned structured contracts, including validation before trusting model/agent output; a separate explicit lifecycle service builds `PromptAgentDefinition` from centralized instructions, creates a version through the current Foundry project SDK, invokes it through the Responses API agent reference, and returns sanitized validation metadata | `src/app/services/foundry_agent_deployment.py`, `scripts/deploy_foundry_agent.py`, `src/app/services/nurse_intake_agent_instructions.py`, `src/app/services/foundry_agent_contract.py`, `tests/test_foundry_agent_deployment.py`, `tests/test_foundry_agent_deployment_script.py` | Boundary, fake-client seam, and lazy live adapter implemented; live Foundry extraction deferred; live version creation/invocation requires manual `--live --json` execution |
...
| Reusable Foundry infrastructure | One Bicep module defines an Entra-oriented AIServices account, child project, and explicitly parameterized model; full-stack and disposable entry points reuse it | `infra/modules/foundry.bicep`, `infra/main.bicep`, `infra/foundry-only.bicep`, `scripts/deploy_foundry_infra.py` | Locally compiled; live deployment remains pending explicit operator execution |
