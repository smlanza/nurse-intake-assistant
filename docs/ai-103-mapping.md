...
| Managed-identity and RBAC readiness | Optional IaC defines a Linux Azure Web App with a system identity and a separate explicit project-scoped Foundry Agent Consumer assignment. Deployment and read-only verification are distinct injected-runner boundaries: the verifier proves the exact principal, role, and project scope and rejects inherited broader access, ambiguity, and malformed data while serializing no identifiers | `infra/modules/web-app.bicep`, `infra/foundry-agent-consumer-rbac.bicep`, `infra/modules/foundry-agent-consumer-rbac.bicep`, `src/app/services/foundry_agent_consumer_rbac_deployment.py`, `scripts/deploy_foundry_agent_consumer_rbac.py`, `src/app/services/foundry_agent_consumer_rbac_verification.py`, `scripts/verify_foundry_agent_consumer_rbac.py`, `tests/test_foundry_agent_consumer_rbac_verification.py`, `tests/test_verify_foundry_agent_consumer_rbac_script.py` | Deployment and assignment-verification readiness are implemented and tested offline only. No live preview, deployment, assignment read, managed-identity token acquisition, hosted Foundry verification, or invocation occurred; this makes no hosted-authentication or invocation claim. Least privilege remains project-scoped and Consumer-only; human nurse review, mock defaults, and suppressed hosted notifications remain unchanged |
...
- Live RBAC deployment, live read-only assignment verification, and hosted
