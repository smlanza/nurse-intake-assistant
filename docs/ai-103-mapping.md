...
| Azure service integration boundaries | Cosmos repository and container factory with point reads/upserts plus cross-partition filtered case-list queries; ACS Email/SMS boundaries; a typed sanitized intake telemetry boundary with a no-op default, lazy Azure Monitor adapter, and production-composed standalone ingestion proof; Bicep baseline for Cosmos, storage, Log Analytics, Application Insights, and optional Azure Web App hosting | `src/app/services/cosmos_case_repository.py`, `src/app/services/cosmos_container_factory.py`, `src/app/services/email_notification_sender.py`, `src/app/services/sms_notification_sender.py`, `src/app/models/intake_telemetry.py`, `src/app/services/azure_monitor_intake_telemetry.py`, `src/app/services/application_insights_intake_telemetry_proof.py`, `scripts/smoke_application_insights_intake_telemetry.py`, `infra/main.bicep`, `infra/modules/web-app.bicep`, `infra/README.md` | The smoke is offline-tested with fakes to process one fixed-fictional intake through production composition, persist only in memory, suppress notifications, emit once, and query read-only within one deadline. One supervised attempt stopped fail-closed with `application_insights_resource_mismatch` before approval, composition, intake, emission, or query; no Azure mutation occurred. It is neither clinical validation nor production monitoring. Hosted Web App telemetry, queue-summary and voicemail-idempotency lookup parity, live Cosmos validation, and production hardening are deferred |
...
- The standalone Application Insights ingestion proof is implemented and
  offline-tested; one supervised attempt stopped at resource validation before
  intake or emission, and App Service-hosted telemetry remains unproven
