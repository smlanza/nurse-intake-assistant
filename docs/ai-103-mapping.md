...
| App Service Authentication perimeter | One authoritative Authentication v2 Bicep boundary declares the verified Web App as existing and deploys only its `authsettingsV2` child. Initial Web App creation reuses that definition for disabled-by-default explicit opt-in. Canonical non-secret existing Entra application and tenant IDs are required; authentication and HTTPS are required by default, protected unauthenticated requests return 401, and exactly `/health`, `/version`, and `/demo/status` remain anonymous. The dedicated acceptance boundary preserves READY, exact Web App, hosting-state, artifact, approval, and freshness prerequisites and cannot reconcile the parent site or plan | `infra/modules/web-app-authentication.bicep`, `infra/modules/web-app.bicep`, `infra/modules/app-service-authentication-config-validation.bicep`, `src/app/services/web_app_authentication_verification.py`, `scripts/verify_web_app_authentication.py`, `scripts/accept_web_app_authentication.py`, `tests/test_web_app_authentication_contract.py`, `tests/test_accept_web_app_authentication_script.py` | Implemented and compiled offline. A prior parent-boundary preview was rejected because it predicted parent hosting changes; the corrected existing-parent boundary has not run live. No Entra application, secret, certificate, role, FastAPI auth middleware, live deployment, or sign-in proof exists. Live what-if and acceptance remain separately supervised; authorization remains deferred |
...
- The standalone Application Insights ingestion proof is implemented,
  offline-tested, and live-proven for one fixed-fictional production-composed
  attempt with private fail-closed READY identity handoff, exactly one emission,
  and strict verification of exactly one eligible record. App Service-hosted
