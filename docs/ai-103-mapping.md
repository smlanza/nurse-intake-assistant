...
| Azure AI Foundry / agent orchestration readiness | `FoundryAiService` and `NurseIntakeAgent` provide tested runtime boundaries and application-owned structured contracts, including validation before trusting model/agent output; a separate explicit lifecycle service builds `PromptAgentDefinition` from centralized instructions, inspects the latest SDK version, idempotently creates/reuses/updates it, and returns sanitized provisioning metadata without invocation | `src/app/services/foundry_agent_deployment.py`, `scripts/deploy_foundry_agent.py`, `src/app/services/nurse_intake_agent_instructions.py`, `src/app/services/foundry_agent_contract.py`, `tests/test_foundry_agent_deployment.py`, `tests/test_foundry_agent_deployment_script.py` | Provisioning boundary, fake-client seam, and lazy live adapter implemented; live Foundry extraction deferred; live provisioning remains manual; invocation is a separate explicit smoke |
