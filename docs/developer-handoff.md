...
- A second packaged operation can check or, with explicit live JSON opt-in,
  ...
  run.
- A packaged synchronous combined proof operation composes those exact metadata
  and fixed-fictional invocation boundaries fail-closed while remaining
  execution-mechanism-neutral. Direct App Service SSH preserves its live-proven
  repository-owned lifecycle, two fixed `APP_PATH` probes, one packaged
  non-invoking check, and guaranteed reaping. SSH hosted managed-identity
  execution is unsupported, and no replacement hosted topology is selected. No
  live hosted managed-identity proof has run, no route or additional compute was
