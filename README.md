...
| Foundry Agent provisioning check | `AGENT_PROVIDER=foundry-agent`, Foundry Agent project/name plus `AZURE_AI_FOUNDRY_MODEL_DEPLOYMENT_NAME`; run `scripts/deploy_foundry_agent.py --env-file .env.foundry-agent.local --check`, then explicit `--live --json` only for idempotent create/reuse/update | Application runtime and all notification/storage providers remain mock; provisioning never invokes the agent or runs at startup/intake time |
